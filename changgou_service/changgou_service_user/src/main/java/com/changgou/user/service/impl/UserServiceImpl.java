...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Integer pointsAdd(String username, Integer points) {
        Integer result = userMapper.pointsAdd(username, points);
        return result;
    }
    ...
    public int updateUserPoint(Task task) {
        ...
        //3.将任务存入到redis中
        redisTemplate.boundValueOps(task.getId()).set("exit", 30, TimeUnit.SECONDS);

        //4.修改用户积分
        int result = userMapper.pointsAdd(username, point);
        if (result <= 0) {
            return 0;
        }
        //5.记录积分日志信息
