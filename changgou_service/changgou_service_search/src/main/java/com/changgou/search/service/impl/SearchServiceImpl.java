                ...
                public <T> AggregatedPage<T> mapResults(SearchResponse searchResponse, Class<T> aClass, Pageable pageable) {
                    ...
                    if (hits != null) {
                        ...
                        for (SearchHit hit : hits) {
                            //SearchHit转换为skuinfo
                            SkuInfo skuInfo = parseObject(hit.getSourceAsString(), SkuInfo.class);
    ...
    public Map<String, Set<String>> formartSpec(List<String> specList) {
        ...
        if (specList != null && specList.size() > 0) {
            for (String specJsonString : specList) {
                ...
                Map<String, String> map = JSONObject.parseObject(specJsonString, Map.class);


                for (String specKey : map.keySet()) {
                    Set<String> stringSet = result.get(specKey);
                    if (stringSet == null) {
                        stringSet = new HashSet<>();
                    }
                    stringSet.add(map.get(specKey));
                    result.put(specKey, stringSet);
