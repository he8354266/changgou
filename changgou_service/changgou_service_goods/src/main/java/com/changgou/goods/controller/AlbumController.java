...
public class AlbumController {
    ...
    public Result findList() {
        List<Album> albumList = albumService.findList();
        return new Result(true, StatusCode.OK, "查询成功", albumList);
    }

    @PostMapping(value = "/insert")
    public Result insertByMap(@RequestBody Map insertMap) {
        int result = albumService.insertByMap(insertMap);
        return new Result(true, StatusCode.OK, "新增成功", result);
    }

    @GetMapping(value = "/delete/{id}")
    public Result insertByMap(@PathVariable int id) {
        int result = albumService.deleteAlbum(id);
        return new Result(true, StatusCode.OK, "删除成功", result);
    }

    @PostMapping(value = "/update")
    public Result updateMap(@RequestBody Album album) {
        int result = albumService.updateAlbum(album);
        return new Result(true, StatusCode.OK, "更新成功", result);
    }

    @GetMapping(value = "/findTemplate/{templateId}")
    public Result findTemplate(@PathVariable int templateId) {
        List<Map> result = (List<Map>) albumService.findTemplate(templateId);
